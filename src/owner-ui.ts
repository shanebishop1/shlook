...
const openUpload=()=>{uploadDialog.hidden=false;document.body.classList.add('upload-modal-open');uploadOpen.setAttribute('aria-expanded','true');requestAnimationFrame(()=>(uploadResult.hidden?uploadInput:uploadForm.querySelector('[data-upload-copy]')).focus())};
const closeUpload=()=>{if(uploadBusy)return;uploadDialog.hidden=true;document.body.classList.remove('upload-modal-open');uploadOpen.setAttribute('aria-expanded','false');uploadOpen.focus()};
const uploadVisibility=()=>uploadForm.querySelector('[name="upload-visibility"]:checked').value;
const syncUploadVisibility=()=>{const visibility=uploadVisibility();const labels={private:'Private by default',secret_link:'Secret link',public:'Public'};const buttons={private:'Upload privately',secret_link:'Upload with secret link',public:'Upload publicly'};const hints={private:'Private URLs require owner or configured agent Access.',secret_link:'Anyone with the secret URL can open it. The link is not listed publicly.',public:'Anyone with the public URL can open it.'};uploadForm.querySelector('[data-upload-option-summary]').textContent=labels[visibility];uploadForm.querySelector('[data-upload-hint]').textContent=hints[visibility];uploadSubmit.textContent=buttons[visibility]};
const chooseUploadFile=file=>{if(!file)return;selectedUploadFile=file;const base=file.name.replace(/\\.[^.]+$/,'').trim();uploadName.value=(base||file.name||'Upload').slice(0,80);uploadForm.querySelector('[data-drop-title]').textContent='Ready to upload';const selected=uploadForm.querySelector('[data-upload-selected]');selected.textContent=file.name+' · '+(file.size<1024?file.size+' B':file.size<1048576?(file.size/1024).toFixed(1)+' KB':(file.size/1048576).toFixed(1)+' MB');selected.hidden=false;uploadSubmit.disabled=false;uploadResult.hidden=true;setUploadStatus('Ready. Private by default.')};
...
uploadDialog.addEventListener('click',event=>{if(event.target===uploadDialog)closeUpload()});
uploadDialog.addEventListener('keydown',event=>{if(event.key!=='Tab')return;const focusable=[...uploadDialog.querySelectorAll('button:not(:disabled),input:not(:disabled),textarea:not(:disabled),summary,a[href]')].filter(element=>element.offsetParent!==null);if(focusable.length===0)return;const first=focusable[0];const last=focusable[focusable.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}});
...
export async function ownerPage(
  ...
  <section id="upload-dialog" class="upload-card" data-upload-dialog hidden role="dialog" aria-modal="true" aria-labelledby="upload-heading"><form class="upload-form" data-upload-form data-private-origin="${escapeHtml(privateOrigin)}" data-public-origin="${escapeHtml(publicOrigin)}"><label class="drop-zone" data-drop-zone><input data-upload-input type="file" required><span class="upload-glyph" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M12 16V4m0 0L7.5 8.5M12 4l4.5 4.5M5 14v4a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-4" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span class="drop-copy"><strong data-drop-title>Drop a file here</strong><span>or tap to choose from your computer</span><span class="upload-selected" data-upload-selected hidden></span></span></label><div class="upload-config"><div class="upload-heading"><h2 id="upload-heading">Upload a file</h2><button class="upload-close" type="button" data-upload-close aria-label="Close upload"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg></button></div><p>Private by default. Choose a file and upload with the defaults, or open options first.</p><details class="upload-options"><summary><span>Options</span><span data-upload-option-summary>Private by default</span></summary><div class="upload-fields"><label class="upload-field">Title<input data-upload-name name="upload-name" maxlength="80" required></label><label class="upload-field">Description<textarea data-upload-description name="upload-description" maxlength="500" placeholder="Optional"></textarea></label><fieldset class="upload-visibility"><legend>Visibility</legend><div class="visibility-choices"><label class="visibility-option"><input type="radio" name="upload-visibility" value="private" checked><span class="visibility-choice"><strong>Private</strong><small>You and configured agents</small></span></label><label class="visibility-option"><input type="radio" name="upload-visibility" value="secret_link"><span class="visibility-choice"><strong>Secret link</strong><small>Anyone with the URL</small></span></label><label class="visibility-option"><input type="radio" name="upload-visibility" value="public"><span class="visibility-choice"><strong>Public</strong><small>Open public URL</small></span></label></div></fieldset></div></details><p class="upload-hint" data-upload-hint>Private URLs require owner or configured agent Access.</p><div class="upload-actions"><button class="button primary" type="submit" data-upload-submit disabled>Upload privately</button><span class="upload-status" data-upload-status role="status" aria-live="polite">Choose one file to begin.</span></div></div><div class="upload-result" data-upload-result hidden><div class="upload-result-copy"><span class="upload-result-label" data-upload-result-label>Private URL ready</span><a class="upload-result-url" data-upload-result-url target="_blank" rel="noopener noreferrer"></a></div><div class="upload-result-actions"><button class="button" type="button" data-upload-copy>${copyIcon()} Copy URL</button><button class="button" type="button" data-upload-refresh>Manage in archive</button></div></div></form></section>
