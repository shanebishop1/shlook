...
const chooseUploadFile=file=>{if(!file)return;selectedUploadFile=file;const base=(file.name.replace(/\\.[^.]+$/,'').trim()||file.name||'Upload').slice(0,80);uploadName.value='';uploadName.dataset.fallback=base;uploadName.placeholder=base;uploadForm.dataset.step='options';dropZone.hidden=true;uploadConfig.hidden=false;uploadSubmit.disabled=false;uploadResult.hidden=true;setUploadStatus('');requestAnimationFrame(()=>uploadConfig.querySelector('h2').focus())};
