...
function assetRows(
  ...
  return `<tr class="artifact-row" data-record="${id}" data-search="${id} ${asset.visibility}" data-visibility="${asset.visibility}" aria-selected="false">
    <td class="preview-cell"><button class="preview-button" type="button" data-inspect aria-label="Inspect artifact ${shortId}"><iframe class="preview-frame" src="${privateUrl}" title="Preview of artifact ${shortId}" sandbox="allow-same-origin" loading="lazy" referrerpolicy="no-referrer" tabindex="-1"></iframe></button></td>
  ...
  <tr class="detail-row" id="detail-${id}" data-detail="${id}" hidden>
    <td colspan="7"><div class="inspector">
      <div class="large-preview"><iframe class="preview-frame" src="${privateUrl}" title="Large preview of artifact ${shortId}" sandbox="allow-same-origin" loading="lazy" referrerpolicy="no-referrer" tabindex="-1"></iframe></div>
      <div class="panel">
...
export async function ownerPage(
  ...
  return new Response(body, {
    headers: {
      ...
      "content-security-policy": `default-src 'none'; script-src 'nonce-${nonce}'; style-src 'nonce-${nonce}'; connect-src 'self'; frame-src ${privateOrigin}; base-uri 'none'; form-action 'self'; frame-ancestors 'none'`,
