   ...
   <tr class="detail-row" id="detail-${id}" data-detail="${id}" data-public-url="${publicUrl}" data-secret-url="${secretUrl}" data-has-secret="${asset.has_secret}" hidden>
    <td colspan="7"><div class="inspector">
      ...
      <div class="panel">
        ...
        <div class="confirm" data-confirm hidden role="dialog" aria-modal="true" aria-labelledby="delete-title-${id}" tabindex="-1"><div class="confirm-card"><span class="confirm-eyebrow">Permanent action</span><p id="delete-title-${id}">Delete ${name}?</p><p class="confirm-copy">This artifact and its files will be removed permanently.</p><div class="button-row"><button class="button danger" type="button" data-delete>Delete permanently</button><button class="button" type="button" data-cancel-delete>Cancel</button></div></div></div>
       </div>
    </div></td>
  </tr>`;
}
...
const closeConfirmation=confirmation=>{confirmation.hidden=true;document.body.classList.remove('confirm-open');confirmation.closest('[data-detail]').querySelector('[data-request-delete]').focus()};
...
document.addEventListener('click',event=>{const request=event.target.closest('[data-request-delete]');if(request){event.preventDefault();event.stopImmediatePropagation();const confirmation=request.closest('[data-detail]').querySelector('[data-confirm]');confirmation.hidden=false;document.body.classList.add('confirm-open');requestAnimationFrame(()=>confirmation.querySelector('[data-delete]').focus());return}const cancel=event.target.closest('[data-cancel-delete]');if(cancel){event.preventDefault();event.stopImmediatePropagation();closeConfirmation(cancel.closest('[data-confirm]'));return}const confirmation=event.target.closest('[data-confirm]');if(confirmation&&event.target===confirmation){event.stopImmediatePropagation();closeConfirmation(confirmation)}},true);
...
function uploadRefinementStyles(): string {
  return `.search input:focus-visible{outline:1px solid var(--line-strong);outline-offset:1px;border-color:var(--muted)}.upload-trigger:hover svg{transform:scale(1.12)}.upload-card:focus,.upload-heading h2:focus{outline:0}.upload-form{position:relative;grid-template-columns:1fr}.upload-close{position:absolute;top:12px;right:12px;z-index:2}.drop-zone{min-height:260px;padding-right:72px;border:0}.drop-zone:focus-within{outline:0;box-shadow:inset 0 0 0 1px var(--line-strong)}.upload-config[hidden]{display:none}.upload-config{padding:30px}.upload-heading{padding-right:42px}.upload-field>span{display:inline;color:var(--muted);font-size:10px;font-weight:500;letter-spacing:0;text-transform:none}.visibility-choices{align-items:stretch}.visibility-option{display:block;height:100%}.visibility-choice{height:100%;min-height:68px;transition:border-color .14s ease,background-color .14s ease,box-shadow .14s ease}.visibility-option input:checked+.visibility-choice{border-color:var(--muted);background:var(--accent-soft);box-shadow:inset 0 0 0 1px var(--line-strong)}.upload-status:empty{display:none}@media(max-width:520px){.drop-zone{min-height:180px;padding-right:64px}.upload-config{padding:22px}.visibility-choice{min-height:58px}}@media(prefers-reduced-motion:reduce){.upload-trigger:hover svg{transform:none}.visibility-choice{transition:none}}`;
}
