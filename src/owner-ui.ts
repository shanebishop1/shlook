...
function pencilIcon(): string {
  return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L8 18l-4.7 1.2L4.5 14.5Z" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/><path d="m14.5 5.5 3 3" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>';
}
...
const uploadResult=uploadForm.querySelector('[data-upload-result]');
 ...
 const pencilSvg=selectToggle.innerHTML;
...
const setUploadProgress=message=>{if(uploadProgressLabel)uploadProgressLabel.textContent=message};
const openUpload=()=>{uploadDialog.hidden=false;document.body.classList.add('upload-modal-open');uploadOpen.setAttribute('aria-expanded','true');requestAnimationFrame(()=>uploadDialog.focus())};
 const resetUpload=()=>{selectedUploadFile=undefined;uploadInput.value='';uploadName.value='';uploadName.placeholder='';delete uploadName.dataset.fallback;uploadDescription.value='';uploadForm.dataset.step='file';dropZone.hidden=false;dropZone.style.display='';uploadConfig.hidden=true;uploadProgress.hidden=true;uploadResult.hidden=true;uploadSubmit.disabled=true;uploadForm.querySelector('[name="upload-visibility"][value="private"]').checked=true;setUploadStatus('')};
...
const trapDialogFocus=(dialog,event)=>{if(event.key!=='Tab')return;const focusable=[...dialog.querySelectorAll('button:not(:disabled),input:not(:disabled),textarea:not(:disabled),summary,a[href]')].filter(element=>element.offsetParent!==null);if(focusable.length===0)return;const first=focusable[0];const last=focusable[focusable.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}};
 ...
 const setSelectMode=enabled=>{selectMode=enabled;document.body.classList.toggle('selection-mode',enabled);selectToggle.innerHTML=enabled?'Done':pencilSvg;selectToggle.setAttribute('aria-label',enabled?'Done selecting':'Select artifacts');selectToggle.setAttribute('aria-pressed',String(enabled));selectToggle.classList.toggle('icon-button',!enabled);if(!enabled)document.querySelectorAll('[data-select-item]').forEach(item=>{item.checked=false});syncSelection()};
const uploadVisibility=()=>uploadForm.querySelector('[name="upload-visibility"]:checked').value;
const chooseUploadFile=file=>{if(!file)return;selectedUploadFile=file;const base=(file.name.replace(/\\.[^.]+$/,'').trim()||file.name||'Upload').slice(0,80);uploadName.value='';uploadName.dataset.fallback=base;uploadName.placeholder=base;uploadForm.dataset.step='options';dropZone.hidden=true;dropZone.style.display='none';uploadConfig.hidden=false;uploadSubmit.disabled=false;uploadProgress.hidden=true;uploadResult.hidden=true;setUploadStatus('');requestAnimationFrame(()=>uploadConfig.querySelector('h2').focus())};
...
uploadForm.addEventListener('submit',async event=>{event.preventDefault();const file=selectedUploadFile;if(!file){setUploadStatus('Choose a file first.',true);uploadInput.click();return}if(!uploadForm.reportValidity())return;uploadBusy=true;uploadClose.disabled=true;uploadSubmit.disabled=true;uploadForm.dataset.step='uploading';dropZone.hidden=true;dropZone.style.display='none';uploadConfig.hidden=true;uploadResult.hidden=true;uploadProgress.hidden=false;let id;try{setUploadProgress('Creating upload...');const created=await ownerRequest('/api/assets',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:uploadName.value.trim()||uploadName.dataset.fallback,description:uploadDescription.value.trim()||null})});id=created.asset&&created.asset.id;if(typeof id!=='string')throw new Error('Create response did not include an asset ID.');setUploadProgress('Uploading '+file.name+'...');const uploaded=await ownerRequest('/api/assets/'+id+'/files/'+encodeURIComponent(file.name),{method:'PUT',headers:{'content-type':file.type||'application/octet-stream'},body:file});if(!uploaded.file||typeof uploaded.file.uploadId!=='string')throw new Error('Upload response did not include a file ID.');setUploadProgress('Finishing upload...');await ownerRequest('/api/assets/'+id+'/finalize',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({entrypoint:file.name,files:[uploaded.file]})});const visibility=uploadVisibility();let resultUrl=uploadForm.dataset.privateOrigin+'/assets/'+id+'/';let resultLabel='Private URL ready';if(visibility==='public'){await ownerRequest('/api/assets/'+id+'/visibility',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({visibility:'public'})});resultUrl=uploadForm.dataset.publicOrigin+'/assets/'+id+'/';resultLabel='Public URL ready'}else if(visibility==='secret_link'){const secret=await ownerRequest('/api/assets/'+id+'/secret?mode=create',{method:'POST'});if(typeof secret.url!=='string')throw new Error('Secret-link response did not include a URL.');resultUrl=secret.url;resultLabel='Secret URL ready'}uploadResultUrl.href=resultUrl;uploadResultUrl.textContent=resultUrl;uploadForm.querySelector('[data-upload-result-label]').textContent=resultLabel;uploadForm.dataset.step='done';uploadProgress.hidden=true;uploadConfig.hidden=true;uploadResult.hidden=false;uploadForm.querySelector('[data-upload-copy]').focus()}catch(error){if(id)try{await ownerRequest('/api/assets/'+id,{method:'DELETE'})}catch{}uploadForm.dataset.step='options';uploadProgress.hidden=true;uploadConfig.hidden=false;uploadResult.hidden=true;setUploadStatus(error instanceof Error?error.message:'Upload failed.',true)}finally{uploadBusy=false;uploadClose.disabled=false;uploadSubmit.disabled=false}});
uploadForm.querySelector('[data-upload-copy]').addEventListener('click',async()=>{try{await copyText(uploadResultUrl.href);uploadForm.querySelector('[data-upload-result-label]').textContent='URL copied.'}catch(error){uploadForm.querySelector('[data-upload-result-label]').textContent=error instanceof Error?error.message:'Copy failed.'}});
...
export async function ownerPage(
  ...
  const renderedBody = body
    ...
    .replace(
      ...
      '<div class="toolbar" aria-label="Archive controls"><div class="selection-actions" data-selection-actions><button class="button icon-button" type="button" data-select-toggle aria-label="Select artifacts" aria-pressed="false">${pencilIcon()}</button><button class="button danger icon-button" type="button" data-batch-delete aria-label="Delete selected artifacts" hidden>${trashIcon()}</button></div>',
    )
    .replaceAll(
